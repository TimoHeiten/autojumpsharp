...
public static partial class SqliteStore
{
    ...
    /// <returns></returns>
    public static string? Lookup(IEnumerable<string> args, Lookups lookups)
    {
        using var connection = TouchStore(lookups);
        var location = GetValuesForMatch(args, connection);
        BumpPath(location, connection, lookups);

        return location.Path;
    }

    public static SQLiteConnection TouchStore(Lookups lookups)
    {
        // create a new sqlite db if none exists at the user directory + autojump.db
        var path = lookups.Configuration.ConnectionPath;

        bool isInitial = !File.Exists(path);
        if (isInitial)
        {
            lookups.Log($"Creating new store: {path}");
        ...
        if (isInitial)
        {
            lookups.Log($"setup the table(s) at: {path}");
    ...
    private static void BumpPath(Location location, SQLiteConnection connection, Lookups lookups)
    {
        var (id, path, lastAccessed, count) = location;
        var newCount = lookups.LastAccessedAndCurrentCount_ToNewCount(lastAccessed, count);
